...
void run(unsigned thread_id, Address public_ip, Address private_ip,
  ...
  zmq::socket_t storage_request_puller(context, ZMQ_PULL);
  storage_request_puller.bind(wt.storage_request_bind_address());
  ...
  while (true) {
    ...
    if (pollitems[7].revents & ZMQ_POLLIN) {
      log->info("Received request_response");
      auto work_start = std::chrono::system_clock::now();
...
int main(int argc, char *argv[]) {
  ...
  if (stype != NULL) {
    if (strncmp(stype, "txn", 3) == 0) {
      kSelfTier = Tier::TXN;
    } else if (strncmp(stype, "memory", 6) == 0) {
